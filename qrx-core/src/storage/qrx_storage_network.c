...
int qrx_storage_network_upload_many(const QrxShardProviderSource*s,const char*const*paths,size_t n,const char*cid,size_t*ok){if(!s||!paths||!n||n>QRX_STORAGE_MAX_FETCH_SOURCES||!cid)return -1;qrx_thread_t th[QRX_STORAGE_MAX_FETCH_SOURCES];UpW w[QRX_STORAGE_MAX_FETCH_SOURCES];size_t launched=0;for(size_t i=0;i<n;i++){w[i]=(UpW){&s[i],cid,paths[i],-1};if(qrx_thread_create(&th[i],NULL,up_worker,&w[i]))break;launched++;}size_t good=0;for(size_t i=0;i<launched;i++){qrx_thread_join(th[i],NULL);if(!w[i].rc)good++;}if(ok)*ok=good;return launched==n&&good==n?0:-1;}
