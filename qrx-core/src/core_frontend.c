...
static const QrxProfile PROFILES[] = {
    ...
    {"alpha","QRX Public Alpha","qrx-alpha","9f1ad2e9e8c9f9b8a1d7e2c3456f7890abcdeffedcba09876543210fedcba98700112233445566778899aabbccddeeff11223344556677889900aabbccdd","62","QRXA62",26661,{"127.0.0.1:26661","127.0.0.1:26662","127.0.0.1:26663",NULL},"20","5000","2100000000000000","25000000","0",10,100,524288,8192,30,70,0,1000,0},
...
static int mkdir_p(const char *path){char tmp[PATH_MAX]; size_t len; if(!path||!*path) return -1; snprintf(tmp,sizeof(tmp),"%s",path); len=strlen(tmp); if(len&&tmp[len-1]=='/') tmp[len-1]=0; for(char *p=tmp+1; *p; ++p){ if(*p=='/'){*p=0; if(mkdir(tmp,0700)!=0 && errno!=EEXIST) return -1; *p='/';}} if(mkdir(tmp,0700)!=0 && errno!=EEXIST) return -1; return 0;}
...
void qrx_default_datadir(const char *network, const char *override_datadir, char *out, size_t out_sz){ const char *home=getenv("HOME"); if(!home) home="."; if(override_datadir&&*override_datadir) snprintf(out,out_sz,"%s/%s",override_datadir,network); else snprintf(out,out_sz,"%s/.qrx/%s",home,network); }
